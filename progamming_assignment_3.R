best <- function(state, outcome) {
 ...
 dataset  <- as.data.frame(cbind(data[, 2],   # hospital
                             data[, 7],   # state
                             data[, 11],  # heart attack
                             data[, 17],  # heart failure
                             data[, 23]), # pneumonia
                       stringsAsFactors = FALSE)
 colnames(dataset) <- c("hospital", 
                        "state", 
                        "heart attack", 
                        "heart failure",
                        "pneumonia")

##Check if the state and the outcome are valid
 if (!state %in% dataset[, "state"]){
   stop('invalid state')
 } else if (!outcome %in% c("heart attack", "heart failure", "pneumonia")){
   stop('invalid outcome')
 } else{
   filter_state <- which(dataset[, "state"] == state)
   state_data <- dataset[filter_state,]
   input_outcome <- as.numeric(state_data[, eval(outcome)])
   min_value <- min(input_outcome, na.rm = TRUE)
   result <- state_data[, "hospital"][which(input_outcome == min_value)]
